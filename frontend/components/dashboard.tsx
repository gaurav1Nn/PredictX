...
export function Dashboard() {
  ...
  // Safely calculate values for stats cards
  const calculateBiggestWin = () => {
    if (!bets || bets.length === 0) return "0 ETH";
    try {
      const maxBet = Math.max(...bets.map(b => Number(ethers.formatEther(b.amount))));
      return `${maxBet.toFixed(4)} ETH`;
    } catch (error) {
      console.error("Error calculating biggest win:", error);
      return "0 ETH";
    }
  };

  const calculateAverageBet = () => {
    if (!bets || bets.length === 0) return "0 ETH";
    try {
      const total = bets.reduce((sum, b) => {
        const amount = Number(ethers.formatEther(b.amount));
        return sum + amount;
      }, 0);
      return `${(total / bets.length).toFixed(4)} ETH`;
    } catch (error) {
...
function ActivityList({ bets, filtered }: { bets: any[]; filtered?: "win" | "loss" }) {
  if (!bets || bets.length === 0) {
    return <p className="text-muted-foreground">No activity to display.</p>;
  }
  ...
  const activities = bets.map((bet, index) => {
    ...
    return {
      ...
      outcome: bet.outcomeIndex === BigInt(0) || bet.outcomeIndex === 0 ? "Yes" : "No", 
      amount: `${ethers.formatEther(bet.amount)} ETH`,
      result: "pending",
